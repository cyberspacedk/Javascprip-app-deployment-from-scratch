...
> ! Лишние стили не загружаются, только необходимые для отображения
...
При удалении компоненты удаляется также связынные с ним стили

- Динамическая стилизация  
Стили компоненты можем динамически менять в зависимости от переданных `props` или `theme`
...
Styled Components используют шаблонные строки (template literals) для стилизации компонентов. 
...
В CSS свойство, которое будет зависеть от переданного `props` передаем CB функцию, которая в агрументах получит ссылку на объект `props` компоненты и в зависимости от значений `props` возвращаем из ф-ции необходимый `css`
...
Для CSS-анимации используем `keyframes`
